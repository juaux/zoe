<?php
    ...
    // UPDATE statement (combined and secure)
    if(isset($_POST['id'])){
        $id = $_POST['id'];
        $nome = $_POST['nome'];
        ...
        $sql = "UPDATE professores SET nome=?, telefone=?, email=?, data_nasc=?, cpf=?, cep=?, endereco=?, bairro=?, numero=?, cidade=?, estado=?, uf=?, sexo=?, curso=?, formacao=?, data_entrada=?, responsavefotol=? WHERE id=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("nome", $nome, $telefone, $email, $data_nasc, $cpf, $cep, $endereco, $bairro, $numero, $cidade, $estado, $uf, $sexo, $curso, $formacao, $data_entrada, $foto, $id);
    ...
    if(isset($_POST['nome'])){
        ...
        $sql = "INSERT INTO professoresdb (nome, telefone, email, data_nasc, cpf, cep, endereco, bairro, numero, cidade, estado, uf, sexo, curso, formacao, dataentrada, foto) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $con->prepare($sql);
